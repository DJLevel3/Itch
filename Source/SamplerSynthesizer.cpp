...
void SamplerSynthesizer::processBlock(juce::AudioBuffer<float>& buffer, int beginSample, int endSample, float beginScratch, float endScratch, float beginVolume, float endVolume) {
    ...
    if ((samples[currentSample].loaded == false)) {
        for (int i = beginSample; i < endSample; i++) {
            buffer.setSample(0, i, 0);
            buffer.setSample(1, i, 0);
        }
    ...
    beginScratch *= samples[currentSample].buffer->getNumSamples() - 1;
    endScratch *= samples[currentSample].buffer->getNumSamples() - 1;
    float time;
    float curVolume;
    int sampleNow = beginSample;
    for (int i = 0; i < endSample - beginSample; i++) {
        time = lerp_f(beginScratch, endScratch, float(i) / (endSample - beginSample));
        curVolume = lerp_f(beginVolume, endVolume, float(i) / (endSample - beginSample));
        for (int c = 0; c < 2; c++) {
            ...
            if (index < samples[currentSample].buffer->getNumSamples() && samples[currentSample].waitingForReset == false) {
                ...
                buffer.setSample(c, sampleNow, curVolume * lerp_f(sampleHere, sampleNext, float(time - index)));
            }
            else {
